...
[yt-dlp 収集] → [字幕パース] → [SQLite FTS5 インデックス] →┬→ [FastAPI 検索 API]        → [フロント + YouTube IFrame]
                                                         └→ [静的サイト書き出し(シャード索引)] → [ブラウザ内検索 (GitHub Pages)]
```
...
- **静的サイト版**は同じ検索ロジックを `web/api.js` がブラウザ内で再現し、シャード索引を検索する
  （n-gram バケットと該当シャードだけを取得。詳細は「ブラウザだけで使う」を参照）。
...
- **スケーリング（索引 version 4）**: 索引はトリグラム転置インデックスをシャーディングして
  書き出します（`static/idx/manifest.json` ＋ `tri/<k>.json` ＋ `bi/<k>.json` ＋ `shard-*.json`）。
  データが増えても検索が重くならないよう、次の3点で「必要な分だけ」取得します。
  - **n-gram 索引はバケット分割**: 全語彙をまとめた 1 ファイルは数百万語彙で数十MBに達し、
    検索前に必ず読む起動コストになります。gram のハッシュで分割し、**クエリに出てくる
    gram のバケットだけ**を取得します（起動時に読むのは manifest だけ）。
  - **シャードは投稿日の新しい順**（shard 0 が最新）。既定の**新着順では先頭から順に見て、
    そのページに必要な件数が埋まった時点で打ち切ります**。よくある語（「おはよう」など）でも
    全シャードを取りに行きません。打ち切った場合の件数は下限なので「37+ 件」のように表示します。
  - **絞り込みはシャードを事前に間引く**: manifest がシャードごとのメンバー/ブランチを持ち、
    該当し得ないシャードは取得しません。
  1〜2文字の検索はトリグラムが使えないため 2-gram 索引／全シャード走査にフォールバックします。
  **一致度順**は全体を見ないと厳密な最良が決まらないため、新しい側から十分な候補を集めた
  時点で順位付けします（結果は partial 扱い）。
