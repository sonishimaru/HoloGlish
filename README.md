...
- 既定は**全ブランチ・6時間ごと・1実行あたり各チャンネル最大30本**。再開可能なので、
  繰り返し実行で未取得分を積み上げ、全アーカイブに到達します（cron はリポジトリ側で調整可）。
  ※上限なしにするとジョブがタイムアウトし公開ステップが走らず収集分が失われるため、1実行を区切っています。
- 手動実行（`workflow_dispatch`）では対象ブランチ・メンバー・本数・待機秒を指定できます
  （大きめの本数を指定すればまとめて取得できますが、ジョブ上限350分に収まる範囲で）。
